...
## Fixes (v2.6.0 — Astra round 3)
Ten verified findings, checks [34]–[43]:
1. **Budget enforced on the final output, every path.** Assembly is extracted into `_assemble_output()` and compress() loops: build → measure the COMPLETE list (summary wrappers, marker, appended user message included) → trim the oldest tail message (re-sanitized) → rebuild, until it fits `window − headroom` (headroom = min(4096, max(1024, window/10))). Applies to successful summaries and rescues alike; previously only rescues pre-trimmed, on part sums that excluded wrappers — an 11K output for a 10K window could pass. A session with only head messages logs an honest error (nothing compactable).
2. **Rescue requires a verified recovery source.** The archive is checked to exist and be non-empty before any rescue; with transcripts disabled or a failed write, an urgent failure now fails OPEN with a loud error — deleting content with no recovery path is worse than a visible overflow. The no-body case (nothing to summarize) also rescues when urgent, instead of skipping.
3. **Pruning preserves the chain root.** `retain` keeps the newest N transcripts PLUS the oldest — every later archive holds earlier history only as summary text, so the root is the only verbatim copy of the earliest messages. Filenames now carry the session id and pruning counts only this session's files (shared directories no longer cross-prune).
4. **`finish_reason="length"` responses are rejected** as summaries (a partial `<analysis>` block once replaced history); the chain falls through to the next candidate or fails open.
5. **Config reloads are complete and per-key validated.** One invalid value (e.g. a string `target_tokens`) no longer aborts the whole load — each setting parses independently; removed keys RESET (`model`, `provider`, `threshold_percent` revert instead of surviving); the threshold is ALWAYS recomputed after a model switch (a stale 200K trigger once survived a switch to a 10K window).
6. **The main fallback pins `api_mode` too** — the resolver otherwise takes the auxiliary config's mode (e.g. `anthropic_messages`) over the main runtime's `chat_completions`.
7. **JSON-syntax secrets redacted**: optional quotes around field names and values (`"password": "…"`, `API_KEY=…`) — the closing quote before the colon used to break the match.
8. **The guard measures the formatted request.** The body is formatted (tool outputs truncated) and measured BEFORE guarding, with the summarizer's reserved output tokens included — a tool-storm body whose actual prompt was ~1K tokens was once guard-skipped on raw-message estimates.
9. **Zero-tail sessions no longer re-ask completed requests**: the last user message is appended only when the original session ends on it, or to repair a dangling trailing tool — an empty tail after a completed assistant answer gets no re-ask.
10. **Coherent continuation instructions**: when NO user message follows the summary, the "respond ONLY to the latest user message" prefix (which contradicted the resume note and left the model no legal action) is replaced by a continuation form keyed off the summary's Current Work section.

## Fixes (v2.6.1 — Astra round 4)
Five verified findings (all reproduced on 480147e before fixing), checks [44]–[48]:
1. **The size invariant survives every append.** The trim loop could only shrink the tail — when the tail emptied, assembly re-appended the (oversized) LATEST USER MESSAGE verbatim and the loop stopped: a 10,906-token output was returned for a 10,000-token window. Now the appended latest request is truncated to the remaining budget when it is the overflow — archive-verified only, with an in-message pointer to its full text — and a final-size postcondition runs on EVERY path (success, rescue, refusal), logging an error whenever the complete output still exceeds the window.
2. **Trimming requires a verified archive on every path, not just rescues.** The universal budget loop ran after successful summaries too — with transcripts disabled it silently deleted tail content that was never sent to the summarizer. The loop now refuses to trim without a verified archive: the tail stays intact and the (possible) overflow is reported loudly. Data loss outranks overflow.
