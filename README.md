...
**Failure ladder** (v2.5.0) — if the summarizer fails, the engine degrades in stages instead of losing the session: (1) retry once with the **main model**, passed explicitly (model/provider/base_url/api_key/api_mode) so `auxiliary.compression` config can't silently reroute the fallback to the same failed summarizer; (2) if the whole chain is down **and** the session sits at ≥95% of the window **and** the transcript archive is verified on disk, **mechanically rescue** — compact with a stub summary pointing at the archive; without a verified archive the engine fails open instead of deleting unrecoverable content; (3) otherwise fail open: messages unchanged, warning logged, backoff with every-5th-turn probes. Truncated responses (`finish_reason="length"`) count as failures, not summaries. On every path — success or rescue — the final assembled output is measured against the window minus response headroom, and the preserved tail is trimmed until it fits — but only when the transcript archive is verified on disk (the tail was never summarized, so cutting it without an archive would delete content outright; the engine keeps it intact and reports the overflow instead). An oversized **latest user message** that re-enters the output after the tail empties is truncated to the remaining budget, with an in-message pointer to its full text in the archive; a final-size postcondition on every path logs an error if the output still exceeds the window. Retention consolidates pruned intermediate archives into the chain root before deleting them, so the union of retained transcripts always covers every archived message. The body guard measures the actual formatted request (tool outputs truncated), not raw messages. A body too large for every window in the chain is skipped up front — except at ≥95% of the window, where it routes into the rescue. If the compacted output still sits above your *threshold*, a post-compact floor warning tells you the un-trimmable floor (system prompt + preserved head/tail) is too big for the configured trigger.
